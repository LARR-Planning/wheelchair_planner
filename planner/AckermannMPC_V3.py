...
class AckermannMPC():
    ...
    def create_mpc_model(self):
        ...
        # parameters which include the initial (in every step) and the reference state of the robot
        ...
        for i in range(self.N):
            obj = obj + ca.mtimes([(X[:, i] - P[3:6]).T, Q, X[:, i] - P[3:6]]) + ca.mtimes([U[:, i].T, R, U[:, i]])
        ...
        opts_setting = {'ipopt.max_iter': 100, 'ipopt.print_level': 0, 'print_time': 0, 'ipopt.acceptable_tol': 1e-8,
                        'ipopt.acceptable_obj_change_tol': 1e-6, 'ipopt.gamma_theta': 1e-2}
    ...
    def solve(self):
        ...
        if self.current_theta != 0:
            ...
            goal_x, goal_y, goal_theta = self.v_max * self.T * self.N, -self.current_y * np.cos(abs(self.current_theta)), 0
        ...
        # t0 = 0
        # set parameter
        c_p = np.concatenate((x0, xs, [[self.current_vel], [self.current_steering]]))
