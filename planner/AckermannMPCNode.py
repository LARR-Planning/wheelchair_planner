...
class MPCAckermannNode:
    def __init__(self, settings_yaml):
        ...
        self.traj_pub = rospy.Publisher("robot_pred_traj", Float32MultiArray, queue_size=1)
    ...
    def callback_nav(self, data: Float32MultiArray):
        # [t_0, y_err, theta_err, x_dot_r, y_dot_r, theta_dot_r, isStop]
        error: Float32MultiArray = data
        t_cur, y_err, theta_err, x_dot_r, y_dot_r, theta_dot_r, isStop = error.data
        # print(f"error y : {error_y}, error_theta : {error_theta}, time : {error.header.stamp}")
        self.mpc_model.state_update(y_err, theta_err, sqrt(x_dot_r ** 2 + y_dot_r ** 2), theta_dot_r, isStop,
                                    self.with_chair)
    ...
    def control_callback(self, data: rospy.timer.TimerEvent):
        ...
        cur_time = (((data.current_expected.to_sec() % 100000.0) * 1000) // 1.0) / 1000
        command_vecter = Vector3()
        command_vecter.x = np.interp(cur_time, self.control_seq[:, 0], self.control_seq[:, 1])
        command_vecter.y = np.interp(cur_time, self.control_seq[:, 0], self.control_seq[:, 2])
        command_vecter.z = np.interp(cur_time, self.control_seq[:, 0], self.control_seq[:, 3])
        # print(self.control_seq[0, 0]-cur_time)

        # command_vecter.x = self.control_seq[0,1]
        # command_vecter.y = self.control_seq[0,2]
        # command_vecter.z = self.control_seq[0,3]

        self.command_pub.publish(command_vecter)
